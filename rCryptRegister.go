...
func rCryptRegister(w http.ResponseWriter, r *http.Request, db *bolt.DB, conf CoreConf) {
	...
	if r.Method == "POST" {
		...
		if err == nil {
			account.PublicKeyFingerprint, err = VerifyGPGPublicKey(clientRequest.PublicKey)
			account.PublicKey = clientRequest.PublicKey
			account.ByteBudget = 2048

			log.Printf("A new account with fingerprint %s was just created", account.PublicKeyFingerprint)
			...
			if err == nil {
				...
				if err == nil {
					...
					response.Message = "There was an issue creating your account"
					log.Print(err.Error())
				}
			} else {
				response.StatusCode = http.StatusBadRequest
				response.Success = false
				response.Message = "There was an issue verifying your GPG public key - ensure it is ASCII armoured"
				log.Print(err.Error())
